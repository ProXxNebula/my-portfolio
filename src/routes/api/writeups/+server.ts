...
export const GET: RequestHandler = async () => {
  try {
    ...
    const modules = import.meta.glob('/static/writeups-content/*.md', { query: '?raw', import: 'default', eager: true });

    const writeups = Object.entries(modules).map(([filepath, content]) => {
